...
import { ArrowLeft, User as UserIcon, Weight, Mail, Lock, Eye, EyeOff, CheckCircle } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      // Crear usuario directamente en el store
      appStore.createUser(formData.name.trim(), parseFloat(formData.weight));
      // Seleccionar el usuario recién creado
      const newUser = appStore.getState().users.find(u => u.name === formData.name.trim() && u.weight === parseFloat(formData.weight));
      if (newUser) {
        appStore.selectUser(newUser.id);
